  ...
  return (
    <header className='relative flex justify-between items-center p-4 bg-white shadow-md'>
      ...
      <nav className='hidden md:flex space-x-4'>
        {routes.map((item) => (
          <Link key={item.href} href={item.href} className='text-gray-700 hover:text-blue-500'>
            {item.label}
          </Link>
        ))}
      </nav>
      ...
      {isOpen && (
        <nav className='absolute top-16 left-0 w-full bg-white shadow-lg z-10 md:hidden'>
          {routes.map((item) => (
            <Link key={item.href} href={item.href} className='block px-4 py-2 text-gray-700 hover:bg-gray-200'>
            {item.label}
            </Link>
          ))}
